                     ...
                     int       nCmdShow)
{
...
#endif
	if(FAILED(::CoInitialize(NULL))) //JP-MOD
	{
		::MessageBox(NULL, TEXT("COMの初期化に失敗しました。"), TEXT("PeerCast"), MB_OK | MB_ICONEXCLAMATION);
		return -1;
	}
	...
	if (mask & ServMgr::NT_APPLAUSE) //JP-MOD
		CheckMenuItem(trayMenu,ID_POPUP_SHOWMESSAGES_APPLAUSE,MF_CHECKED|MF_BYCOMMAND);

	// Main message loop:
	BOOL bResult;
	while ((bResult = GetMessage(&msg, NULL, 0, 0)) != 0) // fix: GetMessage(&msg, NULL, 0, 0)
	{
		if(bResult == -1)
			break;
